...
observation_blob | No | Varchar | Extra information about an observation | <p>For labs measurements (`concept_cd` like 'LOINC:%'), this contains a semi-colon-separated list of labs attribute key-value pairs: '.*reference_low:([^;]+);reference_high:([^;]+);[^;]+;proc_id:([^;]+);proc_name:([^;]+);[^;]+;[^;]+;component_name:([^;]+)'.</p><p>For medications/drug_exposure, this contains a semi-colon-separated list of medication attribute key:value pairs, the used attributes being: 'refills' (5th position), 'stop_reason' (8), 'sig' (9), 'dose' (10), 'med_route' (11), and 'rxnorm_term_type' (12th position).  These attributes (except 'rxnorm_term_type') correspond to the following fields in the PEDSnet `drug_exposure` table: refills, stop_reason, sig, effective_drug_dose, and route_source_value.  The 'rxnorm_term_type' value is used as an inclusion criterion: rows with a value of 1 (ingredient), 2 (precise ingredient), 3 (multiple ingredients), 9 (semantic clinical drug), or 14 (semantic branded drug) are included in the `drug_exposure` table (as long as `concept_cd` is like 'RXNORM:%' and `modifier_cd` = '@').</p>
