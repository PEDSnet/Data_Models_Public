...
Field |Required | Data Type | Description | PEDSnet Conventions
 --- | --- | --- | --- | ---
concept_cd | Yes | Varchar | Type/concept of observation | Possible values: <ul><li>'ICD9:\*' (for condition/diagnosis, i.e. `modifier_cd` in 'PDX:P', 'PDX:OT', 'DX_SOURCE:HC')</li><li>'ICD9:\*', 'CPT:\*', 'HCPCS:\*', 'MSDRG:\*', or 'CMSDRG:\*' (for procedure, i.e. `modifier_cd` equal to 'ORIGPX:MAPPED')</li><li>'DIASTOLIC', 'SYSTOLIC', 'WT', 'HT', or 'ORIGINAL_BMI' (numerical measurements; `modifier_cd` irrelevant).</li><li>'LOINC:\*' (for labs -- `modifier_cd` irrelevant)</li><li>'RXNORM:*' (for medications/drug_exposure; `modifier_cd` is '@' in this case)</li></ul>  <p>The coding system entries (e.g. 'XXX:YYY') have values YYY that are valid for the specified coding system XXX.  The coding system values must exist as valid `concept_code` values in the PEDSnet V2 vocabulary concept table, constrained by the appropriate `vocabulary_id` (e.g. 'DRG' for DRG rows, 'RxNorm' for RXNORM rows), `concept_class_id` ('DRG' for CMDRG, 'MS-DRG' for MSDRG), and `invalid_reason` ('D' for CMSDRG, NULL for MSDRG).</p>
...
location_cd | No | Varchar | In contrast to the original i2b2 semantics, for PEDSnet medication observations, this field stores the type of drug exposure| For medication rows (`concept_cd` like 'RXNORM:%'), this can take on the following values: <ul><li>ACT\|MED_ORDER_TYPE:I - Inpatient administration (MAR entries): concept_id = 38000180</li><li>ACT\|MED_ORDER_TYPE:A - Prescription written: concept_id = 38000177</li><li>Anything else - concept_id = 44814650 (other)</li></ul>
modifier_cd | No | Varchar | Modifier code for a concept | Possible values: <ul><li>For condition/diagnosis: 'PDX:P' (primary condition), 'PDX:OT' (secondary condition), or 'DX_SOURCE:HC' (EHR problem list entry).</li><li>For procedure: value must be 'ORIGPX:MAPPED'.</li><li>For medications/drug_exposure: '@'</li></ul>
nval_num | No | Varchar | A numerical value | Contains a numerical value when `valtype_cd` = 'N'
observation_blob | No | Varchar | Extra information about an observation | <p>For **labs measurements** (`concept_cd` like 'LOINC:%'), this contains a semi-colon-separated list of labs attribute key-value pairs.  At least the first 11 pairs must be present for PEDSnet purposes, although only the ones marked with '\*' are used -- the others can be placeholders. Key-value pairs for measurements: <ol><li>specimen_taken_time<li>panel_proc_id<li>panel_proc_code<li>\* reference_low (`measurement`.`range_low`)<li>\* reference_high (`measurement`.`range_high`)<li>result_in_range_yn<li>\* proc_id (1st part of `measurement_source_value`)<li>\* proc_name (2nd part of `measurement_source_value`)<li>lab_px<li>component_id<li>\* component_name (3rd part of `measurement_source_value`)<li>abn_ind<li>priority<li>specimen_source</ol></p><p>Similarly, for **medications**/drug_exposure, this contains a semi-colon-separated list of medication attribute key:value pairs.  At least 12 key-value pairs must be specified, although only a few of them (marked with '\*') are used by PEDSnet currently.  The destination PEDSnet fields in the `drug_exposure` table are given in parentheses.  <ol><li>medication_id<li>medication_name<li>ordering_date<li>start_date<li>\* refills (`refills`)<li>frequency<li>prn_yn<li>\* stop_reason (`stop_reason`)<li>\* sig (`sig`)<li>\* dose (`effective_drug_dose`)<li>\* med_route (`route_source_value`)<li>\* rxnorm_term_type<li>order_med_id</ol>  The 'rxnorm_term_type' value is used as an inclusion criterion: rows with a value of 1 (ingredient), 2 (precise ingredient), 3 (multiple ingredients), 9 (semantic clinical drug), or 14 (semantic branded drug) are included in the `drug_exposure` table (as long as `concept_cd` is like 'RXNORM:%' and `modifier_cd` = '@').</p>
...
tval_char | No | Varchar | Generally, a text value or modifier of an observation.  For medications/drug_exposure, this field stores the *quantity* of medication administered.  | Generally (e.g. for measurements), when `valtype_cd` = “T”, stores the text value of the observation.  When `valtype_cd` = “N”: <ul><li>E = Equals</li><li>NE = Not equal</li><li>L = Less Than</li><li>LE = Less than and Equal to</li><li>G = Greater Than</li><li>GE = Greater than and Equal to</ul>).<p>For drug_exposure/medications, this field will be used to populate the `quantity` field of the `drug_exposure` table.</p>
units_cd | No | Varchar | Units in which a numeric measurement or drug dose is expressed | Possible values: For measurements: <ul><li>'in' or 'cm' (for `concept_cd` 'HT').</li><li>'lbs' or 'kg' (for `concept_cd` 'WT').</li><li>'mm Hg' (for `concept_cd` 'SYSTOLIC' or 'DIASTOLIC')</li></ul><p>For 'in' and 'lbs', case is significant.</p><p>For medications/drug_exposures, `units_cd` should be a concept_name (case-insensitive) in the PEDSnet vocabulary in the 'SNOMED' vocabulary and 'Unit' domain.</p>
update_date | No | Datetime | Date the row was updated by the source system (date is obtained from the source system) | Not used.
upload_id | No | Integer | A numeric id given to the upload | Not used.
