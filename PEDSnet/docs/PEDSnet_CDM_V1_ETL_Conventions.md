...
| provider\_source\_value  | Yes | The identifier used for the provider in the source data, stored here for reference.  | Insert a pseudo-identifier (random number, encrypted identifier) into the field. Do not insert the actual PROVIDER\_ID from your site. A mapping from the pseudo-identifier for provider\_source\_value in this field to a real provider ID from the source EHR must be kept at the local site. This mapping is not shared with the data coordinating center. It is used only by the site for re-identification for study recruitment or for data quality review. Sites may consider using the provider\_id field value in this table as the pseudo-identifier as long as a local mapping from provider\_id to the real site identifier is maintained. |
