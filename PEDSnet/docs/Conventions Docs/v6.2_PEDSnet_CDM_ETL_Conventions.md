# ETL Conventions for use with PEDSnet CDM v6.2 OMOP V5.4
...
Version v6.2 of the PEDSnet CDM reflects the ETL processes developed after several iterations of network development. As such, it proposes to align with version 7.0 of the PCORnet CDM.
...
2. The PEDSnet CDM v6.2 is based on the Observational Medical Outcomes Partnership (OMOP) data model, version 5.4. 
...
6. Concept IDs are taken from OMOP 5 vocabularies for the PEDSnet CDM, using the complete (restricted) version that includes licensed terminologies such as CPT and others.
...
#### [1.1 Person](v6.2_PEDSnet_CDM_ETL_Conventions.md#11-person-1)

#### [1.2 Death](v6.2_PEDSnet_CDM_ETL_Conventions.md#12-death-1)

#### [1.3 Location](v6.2_PEDSnet_CDM_ETL_Conventions.md#13-location-1)

#### [1.4 Care Site](v6.2_PEDSnet_CDM_ETL_Conventions.md#14-care_site)

#### [1.5 Provider](v6.2_PEDSnet_CDM_ETL_Conventions.md#15-provider-1)

#### [1.6 Visit Occurrence ](v6.2_PEDSnet_CDM_ETL_Conventions.md#16-visit_occurrence)

#### [1.7 Condition Occurrence](v6.2_PEDSnet_CDM_ETL_Conventions.md#17-condition_occurrence)

#### [1.8 Procedure Occurrence](v6.2_PEDSnet_CDM_ETL_Conventions.md#18-procedure_occurrence)

#### [1.9 Observation](v6.2_PEDSnet_CDM_ETL_Conventions.md#19-observation-1)

#### [1.10 Observation Period](v6.2_PEDSnet_CDM_ETL_Conventions.md#110-observation_period)

#### [1.11 Drug Exposure](v6.2_PEDSnet_CDM_ETL_Conventions.md#111-drug_exposure)

#### [1.12 Measurement](v6.2_PEDSnet_CDM_ETL_Conventions.md#112-measurement-1)

#### [1.13 Fact Relationship](v6.2_PEDSnet_CDM_ETL_Conventions.md#113-fact_relationship)

#### [1.14 Visit Payer](v6.2_PEDSnet_CDM_ETL_Conventions.md#114-visit_payer)

#### [1.15 Measurement Organism](v6.2_PEDSnet_CDM_ETL_Conventions.md#115-measurement_organism)

#### [1.16 ADT Occurrence](v6.2_PEDSnet_CDM_ETL_Conventions.md#116-adt_occurrence)

#### [1.17 Immunization](v6.2_PEDSnet_CDM_ETL_Conventions.md#117-immunization-1)

#### [1.18 Device Exposure](v6.2_PEDSnet_CDM_ETL_Conventions.md#118-device_exposure)

#### [1.19 Location History](v6.2_PEDSnet_CDM_ETL_Conventions.md#119-location_history)

#### [1.20 Hash Token](v6.2_PEDSnet_CDM_ETL_Conventions.md#120-hash_token)

#### [1.21 Specialty](v6.2_PEDSnet_CDM_ETL_Conventions.md#121-specialty-1)

#### [1.22 Location FIPS](v6.2_PEDSnet_CDM_ETL_Conventions.md#122-location_fips)

#### [1.23 Cohort](v6.2_PEDSnet_CDM_ETL_Conventions.md#123-cohort-1)

#### [1.24 Cohort Definition](v6.2_PEDSnet_CDM_ETL_Conventions.md#124-cohort_definition)

#### [Appendix](v6.2_PEDSnet_CDM_ETL_Conventions.md#a1-abms-specialty-category-to-omop-v5-specialty-mapping)
...
- See [Note 11 of Observation](v6.2_PEDSnet_CDM_ETL_Conventions.md#19-observation-1) for more details.

### Note 3:

If maternal and delivery information exists in your source EHR system, and both the mother and child are within the PEDSnet inclusion criteria, then a mother-child relationship can be defined using the [Fact Relationship](v6.2_PEDSnet_CDM_ETL_Conventions.md#113-fact_relationship) table as follows:
...
`gender_concept_id` | Yes |Yes|  Integer |  A foreign key that refers to a standard concept identifier in the Vocabulary for the gender of the person. | Please include valid concept ids (consistent with OMOP CDMv5.4). Predefined value set (valid concept\_ids found in CONCEPT table select \* from concept where ((domain\_id='Gender' and concept\_class\_id='Gender')or (domain\_id='Observation' and vocabulary\_id='PCORNet' and concept\_class\_id in ('Gender','Undefined'))) and concept\_code not in ('Sex\-F','Sex\-M') and invalid\_reason is null: <ul><li>Ambiguous: concept\_id = 44814664 </li> <li>Female: concept\_id = 8532</li> <li>Male: concept\_id = 8507</li> <li>No Information: concept\_id = 44814650 (Vocabulary\_id='PCORNet')</li> <li>Unknown: concept\_id = 44814653</li> <li>Other: concept\_id = 44814649</li></ul>
...
`ethnicity_concept_id` | Yes | Yes|  Integer | A foreign key that refers to the standard concept identifier in the Vocabulary for the ethnicity of the person. | <p>For PEDSnet, a person with Hispanic ethnicity is defined as "A person of Cuban, Mexican, Puerto Rican, South or Central American, or other Spanish culture or origin, regardless of race."</p> Please include valid concept ids (consistent with OMOP CDMv5). Predefined value set (valid concept\_ids found in CONCEPT table where vocabulary\_id ='Ethnicity' or (vocabulary\_id=PCORNet and concept\_class\_id='Undefined) where noted): <ul><li>Hispanic: concept\_id = 38003563</li> <li>Not Hispanic: concept\_id = 38003564</li> <li>No Information: concept\_id = 44814650 (vocabulary\_id='PCORNet')</li> <li>Unknown: concept\_id = 44814653 (vocabulary\_id='PCORNet')</li> <li>Other: concept\_id = 44814649 (vocabulary\_id='PCORNet')</li></ul>
...
`provider_id` | No |Provide When Available| BigInteger |  Foreign key to the primary care provider the person is seeing in the provider table.| <p>For PEDSnet CDM: Sites will use site-specific logic to determine the best primary care provider and document how that decision was made (e.g., billing provider).</p>
...
`visit_end_date` | No |Provide When Available| Date | The end date of the visit. | <p>No date shifting. Full date.</p> <p>If this is a one-day visit the end date should match the start date.</p> <p>**NOTE:** If the encounter is on-going at the time of ETL, this should set to equal the `visit_start_date`.</p>
...
`visit_end_datetime` | Yes |Yes| Datetime | The end date of the visit. | <p>No date shifting.</p> <p>If this is a one-day visit the end date should match the start date.</p> <p>**NOTE:** If the encounter is on-going at the time of ETL, this should set to equal the `visit_start_datetime`.</p>  <p>Full date and time. **If there is no time associated with the date assert 11:59:59 pm for the end time**</p>
...
IMO Description | Direct SNOMED | Via ICD-CM 
--- | --- | --- 
...
Clarity Table	|Source INI|Billing or Claim or Ordering| condition\_type\_concept\_id(s)
---|---|---|---
...
`procedure_concept_id` | Yes |Yes| Integer | A foreign key that refers to a standard procedure concept identifier in the Vocabulary. | <p> All concepts are expected to be standard and valid (I.E. `standard_concept = 'S'` AND `current date < valid_end_date` AND `invalid_reason is NULL`). Concepts are primarily expected to belong to the "Procedure" domain, but may belong to other domains such as "Measurement" for lab orders. Procedure Concepts are based on a variety of vocabularies including but not limited to ICD\-9\-Procedures (vocabulary_id ='ICD9Proc'), ICD\-10\-Procedures (vocabulary\_id ='ICD10PCS'), CPT\-4 (vocabulary\_id ='CPT4'), HCPCS (vocabulary\_id ='HCPCS'), and SNOMED (vocabulary\_id ='SNOMED').</p> <p>Procedures are expected to be carried out within one day. If they stretch over a number of days, such as artificial respiration, usually only the initiation is reported as a procedure (CPT-4 "Intubation, endotracheal, emergency procedure").</p> <p>Procedures could involve the administration of a drug, in which case the procedure is recorded in the procedure table and simultaneously the administered drug in the drug table.</p> <p> **See Note 1 for additional details.**</p>
`modifier_concept_id` | No |Provide When Available| Integer | A foreign key to a standard concept identifier for a modifier to the procedure (e.g. bilateral) |  <p>Valid Modifier Concepts belong to the "Modifier" concept class. </p> <p>select \* from concept where concept\_class\_id like '%Modifier%'. </p>
...
- Procedure Concepts are based on a variety of vocabularies: ICD-9-Proc, ICD-10-Proc, CPT-4, HCPCS, OPCS-4, SNOMED, etc.
...
- Improve Care Now (ICN) IBD Survey Data elements (See Note 10)

**Table 1: Valid Observation concept IDs and Value as concept IDs for PEDSnet v6.2.** 
...
Social Determinants of Health (SDOH) Surveys |See **Note 10** for details| LOINC | | LOINC |
Patient Reported Outcome (PRO) Surveys |See **Note 10** for details | LOINC | | LOINC |
Improve Care Now (IBD) Survey Elements| **See Note 9**|||
...
Below are examples of how the observation table and the fact relationship table would be populated for tobacco, smoking and tobacco type scenarios. In the case where tobacco information is recorded at a visit but there is missing information for tobacco, smoking or tobacco type please assert. The PEDSnet standard relationship concept id for linking tobacco items will be *0*. This concept id was chosen as there was not a specific concept id that exists in the standard vocabulary that adequately defined an appropriate relationship for linking the tobacco items.
...
`observation_concept_id` | Yes |Yes| Integer | A foreign key to the standard observation concept identifier in the Vocabulary. | Lab results and vitals are not stored in this table but are stored in the Measurement table. <br> <p>For any records representing unmapped SDOH or PRO survey question results, set to `0` </p>
...
`person_id` | Yes |Yes|BigInteger | A foreign key identifier to the person who is experiencing the condition. The demographic details of that person are stored in the person table.
`drug_concept_id`| Yes |Yes| Integer | A foreign key that refers to a standard drug concept identifier in the Vocabulary. | Valid drug concept IDs are mapped to RxNorm using the source to concept map table to transform source codes (GPI, NDC etc to the RxNorm target). In the event of multiple RxNorm mappings please see Note 5. See note 7 for guide.
...
**Priority fields for v6.2 submission — Drug Metadata:**
Sites are asked to prioritize completeness for the following DRUG_EXPOSURE fields where source data is available:
- **Dose Unit** (`dose_unit_concept_id` / `dose_unit_source_value`): Populate with the appropriate UCUM concept for the unit of the administered or prescribed dose. Retain the original source unit string in `dose_unit_source_value`.
- **End Dates and Duration** (`drug_exposure_end_date` / `days_supply`): Populate `drug_exposure_end_date` when an end date is available in the source system. Populate `days_supply` when a days-supply value is available (particularly for outpatient prescriptions). When only a start date is available, calculate `drug_exposure_end_date` as `drug_exposure_start_date + days_supply - 1` where possible.
...
- All available component Level Labs. The Lab Listing and PEDSNet LOINC Mapping can be found [here] (https://github.com/PEDSnet/Data_Models_Public/blob/master/PEDSnet/docs/PEDSnet_Component_Loinc_Mapping.xlsx)
...
**Note 8**: Starting in v6.2, please include the following pain scales in the measurement table if available at your site:
...
--- | --- | --- | --- | --- | --- | --- 
Numeric Pain Score | 4022240 | 44818701 | [0-10] | The pain scale display name as it appears in flowsheets | The source value representing the result | Provide when available 
...
If the result of a pain score is recorded as 0, please include the score with a `value_as_number` of 0. It is known that some systems will not prompt for a score if no pain is present. 

All pain scores are expected to have something in `value_source_value`. If the result is a numeric value with an operator, include both the numeric value and operator. If the result is stored as text (e.g. “moderate”), just submit the text in the `value_source_value` field. You do not need to map the term to a `value_as_concept_id` 
...
- Blood Pressure Systolic and Diastolic Blood Pressure Values will be mapped using the fact relationship table. See [Note 2 in the Measurement section](v6.2_PEDSnet_CDM_ETL_Conventions.md#measurement-note-2) for instructions.
- ER Visits that result in an Inpatient Encounter will be mapped using the fact relationship table. See [Additional Notes in the Visit Occurrence section](v6.2_PEDSnet_CDM_ETL_Conventions.md#161-additional-notes) for instructions.
- Tobacco, smoking and tobacco type associations will be mapped using the fact relationship table. See [Note 4 in the Observation section](v6.2_PEDSnet_CDM_ETL_Conventions.md#observation-note-4) for instructions.
- The inpatient medication orders and administrations linking is ***optional***. See [Note 8 in the Drug_Exposure section](v6.2_PEDSnet_CDM_ETL_Conventions.md#111-drug-exposure-1)
- Ventilaors, Ventilator Settings and Ventilator Measurements will be mapped using the fact relationship tbale. See the [CDM Ventilator Pilot Guidance](https://github.com/PEDSnet/Data_Models_Public/blob/master/PEDSnet/docs/CDM%20Ventilator%20Pilot/%20Ventilator%20Pilot%20Guidance.md).
- Mother Child relationships will be mapped using the fact relationship table. See [Note 3 in the Person section](v6.2_PEDSnet_CDM_ETL_Conventions.md#11-person-1) for instructions.
<br>
...
To record cardiac stent information to support the **NEST Cardiac studies**, please use the guidance located [here](https://github.com/PEDSnet/Data_Models_Public/blob/master/PEDSnet/docs/NEST/NEST-CardiacStents.md) to insert records into the **device_exposure** table.

**Note 3**:

For sites particpating in the **CDM Ventilator Pilot**, please use the guidance located [here](https://github.com/PEDSnet/Data_Models_Public/blob/master/PEDSnet/docs/CDM%20Ventilator%20Pilot/Ventilator%20Pilot%20Guidance.md) to insert records into the **device_exposure** table.
...
`device_concept_id`|	Yes|	Yes|	Integer|	A foreign key that refers to a Standard Concept identifier in the Standardized Vocabularies belonging to the 'Device' domain.| **Please use one of the above guidance for mechnical ventilation and cardiac stents. For all other devices, use concept_id = 0.**
...
`entity_id` | Yes |Yes| BigInteger| The unique identifier for the entity. References either person_id, provider_id, or care_site_id, depending on domain_id.|Only patient address histories are present in this table. Due to this, please populate this field with the corresponding person_id.
...
`domain_id`|Yes|Yes|Varchar|The domain of the entity that is related to the specialty. Either PROVIDER or CARE_SITE.|
...
`specialty_concept_id`|Yes|Yes|Integer|A foreign key that refers to the given PROVIDER or CARE_SITE specialty.|See the specifications for the `specialty_concept_id` fields in either the [PROVIDER](https://github.com/PEDSnet/Data_Models_Public/blob/master/PEDSnet/docs/Conventions%20Docs/v6.2_PEDSnet_CDM_ETL_Conventions.md#15-provider-1) or [CARE\_SITE](https://github.com/PEDSnet/Data_Models_Public/blob/master/PEDSnet/docs/Conventions%20Docs/v6.2_PEDSnet_CDM_ETL_Conventions.md#14-care_site) specs for detailed guidance on which concept\_id's to use for this mapping (whichever one is relevant for the given `domain_id` you are inserting data for).<p>**NOTE**: The `specialty_concept_id` for the Primary Specialty (i.e. the `entity_type_concept_id` = 4114681) record should match the value of the `specialty_concept_id` for the associated PROVIDER or CARE\_SITE table record.</p><p>If no specialty information is pressent, or not concept_id mapping seems appropriate, use concept\_id = 38004477 (i.e. "Pediatric Medicine").</p>
...
**Note 2:**
> * The geocode_block field (representing last three digits of a FIPS code) is avaialbe to populate but not required.
...
`subject_id` | Yes | Yes | BigInteger | A foreign key identifier to the Person who is enrolled in the trial. The demographic details of that Person are stored in the PERSON table. | The Local person_id in the context of clinical trial enrollment 
...
For PEDSnet, the current purpose of the COHORT\_DEFINITION table is to define and describe `cohort_definition_ids` included in the COHORT table. COHORT\_DEFINITION  includes a standardized structure for maintaining the inclusion of a subject in a clinical trial cohort.

**Note 1:**
> The ongoing list of cohort\_definition\_ids are maintained by the PEDSnet DCC and are listed in the table below. The current version of the table is also available as a csv (cohort\_definition.csv) both [linked directly here](https://github.com/PEDSnet/Data_Models_Public/blob/master/PEDSnet/docs/Codesets/cohort_definition.csv) and within the PEDSnet Vocabulary.
> 
...
> 
> The cohort_definition table should be treated as an additional "vocabulary lookup table" (similar to the concept for example) and only needs to be retianed locally. Despite being included in the PEDSnet DDL and ETL Conventions, cohort_definition should not be included in your final data submission.
