...
pn_time_of_birth | No | The time of birth at the birth day | <p>Include whatever is the relevant time zone when insert this value.</p> Please keep all accurate/real dates (No date shifting)
...
operator_concept_id| No| A foreign key identifier to the mathematical operator that is applied to the value_as_number.Operators are <, ≤, =, ≥, >| By default the operator concept id should be the concept for '='|
