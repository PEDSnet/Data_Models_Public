...
As for the PEDSnet CDM, these scripts were programmatically generated and are not yet fully tested. Please report any issues [here](https://github.com/PEDSnet/Data_Models/issues).
...
To re-generate the Vocabulary DDL in the various dialects using the sqlalchemy vocab table definition file:

1. Install python requirements with `pip install -r requirements.txt`
2. Run the generation script with `python generate_vocab_ddl_dialects.py`
